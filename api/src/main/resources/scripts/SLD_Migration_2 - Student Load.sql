...
SELECT
    ...
    CASE
        WHEN EXTRACT(MONTH FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) < 9 AND months_between(TO_DATE(EXTRACT(YEAR FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD'))-1 || '0701','YYYYMMDD'), TO_DATE(sld_student.BIRTH_DATE,'YYYYMMDD'))/12 >= 19 THEN 'true'
        WHEN EXTRACT(MONTH FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) >= 9 AND months_between(TO_DATE(EXTRACT(YEAR FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) || '0701','YYYYMMDD'), TO_DATE(sld_student.BIRTH_DATE,'YYYYMMDD'))/12 >= 19 THEN 'true'
        ELSE 'false'
    END as IS_ADULT,
    CASE
        WHEN EXTRACT(MONTH FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) < 9 AND months_between(TO_DATE(EXTRACT(YEAR FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD'))-1 || '1231','YYYYMMDD'), TO_DATE(sld_student.BIRTH_DATE,'YYYYMMDD'))/12 >= 5
                 AND months_between(TO_DATE(EXTRACT(YEAR FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD'))-1 || '0701','YYYYMMDD'), TO_DATE(sld_student.BIRTH_DATE,'YYYYMMDD'))/12 < 19 THEN 'true'
        WHEN EXTRACT(MONTH FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) >= 9 AND months_between(TO_DATE(EXTRACT(YEAR FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) || '0701','YYYYMMDD'), TO_DATE(sld_student.BIRTH_DATE,'YYYYMMDD'))/12 >= 5
                 AND months_between(TO_DATE(EXTRACT(YEAR FROM TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD')) || '0701','YYYYMMDD'), TO_DATE(sld_student.BIRTH_DATE,'YYYYMMDD'))/12 < 19  THEN 'true'
        ELSE 'false'
...
CREATE INDEX SDC_SCHOOL_COLLECTION_ID_INDX
    ON SDC_SCHOOL_COLLECTION(SDC_SCHOOL_COLLECTION_ID);

CREATE INDEX SDC_SCHOOL_COLLECTION_SCHOOL_ID_INDX
    ON SDC_SCHOOL_COLLECTION(SCHOOL_ID);

CREATE INDEX SDC_SCHOOL_COLLECTION_STUDENT_ID_INDX
    ON SDC_SCHOOL_COLLECTION_STUDENT(SDC_SCHOOL_COLLECTION_ID);

CREATE INDEX SDC_SCHOOL_COLLECTION_STUDENT_PEN_INDX
    ON SDC_SCHOOL_COLLECTION_STUDENT(STUDENT_PEN);

CREATE TABLE SDC_SCHOOL_COLLECTION_STUDENT_ENROLLED_PROGRAM
as
SELECT
    LOWER(REGEXP_REPLACE(dbms_crypto.randombytes(16), '(.{8})(.{4})(.{4})(.{4})(.{12})', '\1-\2-\3-\4-\5')) as SDC_SCHOOL_COLLECTION_STUDENT_ENROLLED_PROGRAM_ID,
    (SELECT sdc_coll.SDC_SCHOOL_COLLECTION_ID
     FROM SDC_SCHOOL_COLLECTION sdc_coll, COLLECTION coll, SDC_SCHOOL_COLLECTION_STUDENT sdc_stud
     WHERE sdc_coll.COLLECTION_ID = coll.COLLECTION_ID
       AND sdc_stud.SDC_SCHOOL_COLLECTION_ID = sdc_coll.SDC_SCHOOL_COLLECTION_ID
       AND sdc_stud.STUDENT_PEN = TRIM(sld_student.PEN)
       AND sdc_coll.SCHOOL_ID = (SELECT schl.SCHOOL_ID from SCHOOL schl WHERE SUBSTR( DISTNO||'' ||SCHLNO, 4) = schl.SCHOOL_NUMBER AND SUBSTR( sld_student.DISTNO||'' ||SCHLNO, 0 , 3) = (SELECT dist.DISTRICT_NUMBER from DISTRICT dist WHERE schl.DISTRICT_ID = dist.DISTRICT_ID))
       AND TO_DATE(sld_student.REPORT_DATE,'YYYYMMDD') BETWEEN coll.OPEN_DATE AND coll.CLOSE_DATE) AS SDC_SCHOOL_COLLECTION_STUDENT_ID,
    TRIM(sld_student.ENROLLED_PROGRAM_CODE) AS ENROLLED_PROGRAM_CODE,
    'SLD_MIGRATION' as CREATE_USER,
    sysdate as CREATE_DATE,
    'SLD_MIGRATION' as UPDATE_USER,
    sysdate as UPDATE_DATE
FROM STUDENT_PROGRAMS sld_student;
