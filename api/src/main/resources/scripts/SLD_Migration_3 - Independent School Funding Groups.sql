...
SELECT
    ...
    TO_CHAR(sfc.REPORT_DATE) as REPORT_DATE,
...
INSERT INTO INDEPENDENT_SCHOOL_FUNDING_GROUP_SNAPSHOT (SCHOOL_FUNDING_GROUP_SNAPSHOT_ID, COLLECTION_ID, SCHOOL_ID, SCHOOL_GRADE_CODE, SCHOOL_FUNDING_GROUP_CODE, REPORT_DATE, CREATE_USER, CREATE_DATE, UPDATE_USER, UPDATE_DATE)
SELECT
    LOWER(REGEXP_REPLACE(dbms_crypto.randombytes(16), '(.{8})(.{4})(.{4})(.{4})(.{12})', '\1-\2-\3-\4-\5')) as SCHOOL_FUNDING_GROUP_SNAPSHOT_ID,
    (SELECT COLLECTION_ID FROM COLLECTION coll WHERE TO_DATE(sfc.REPORT_DATE,'YYYYMMDD') BETWEEN coll.OPEN_DATE AND coll.CLOSE_DATE) AS COLLECTION_ID,
    (SELECT schl.SCHOOL_ID from SCHOOL schl WHERE sfc.SCHLNO = schl.SCHOOL_NUMBER AND sfc.DISTNO = (SELECT dist.DISTRICT_NUMBER from DISTRICT dist WHERE schl.DISTRICT_ID = dist.DISTRICT_ID)) as SCHOOL_ID,
