...
import ca.bc.gov.educ.studentdatacollection.api.service.v1.SdcDistrictCollectionStudentSearchService;
...
public class AllStudentLightCollectionGenerateCsvService {
    ...
    private final SdcDistrictCollectionStudentSearchService sdcDistrictCollectionStudentSearchService;

    public AllStudentLightCollectionGenerateCsvService(SdcSchoolCollectionStudentSearchService sdcSchoolCollectionStudentSearchService, SdcDistrictCollectionStudentSearchService sdcDistrictCollectionStudentSearchService) {
        this.sdcSchoolCollectionStudentSearchService = sdcSchoolCollectionStudentSearchService;
        this.sdcDistrictCollectionStudentSearchService = sdcDistrictCollectionStudentSearchService;
    }
    ...
    public DownloadableReportResponse generateDistrict(UUID collectionID) {
        Map<UUID, List<SdcSchoolCollectionStudentLightEntity>> schoolToStudentsMap = sdcDistrictCollectionStudentSearchService.findAllStudentsGroupedBySchoolSynchronous(collectionID);
        CSVFormat csvFormat = CSVFormat.DEFAULT.builder()
        ...
        try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            ...
            for (Map.Entry<UUID, List<SdcSchoolCollectionStudentLightEntity>> entry : schoolToStudentsMap.entrySet()) {
                UUID schoolID = entry.getKey();
                for (SdcSchoolCollectionStudentLightEntity student : entry.getValue()) {
                    String legalFullName = formatFullName(student.getLegalFirstName(), student.getLegalMiddleNames(), student.getLegalLastName());
                    String usualFullName = formatFullName(student.getUsualFirstName(), student.getUsualMiddleNames(), student.getUsualLastName());
                    String feePayer = student.getSchoolFundingCode() != null && student.getSchoolFundingCode().contentEquals("14") ? "Y" : "N";
                    String refugee = student.getSchoolFundingCode() != null && student.getSchoolFundingCode().contentEquals("16") ? "Y" : "N";
