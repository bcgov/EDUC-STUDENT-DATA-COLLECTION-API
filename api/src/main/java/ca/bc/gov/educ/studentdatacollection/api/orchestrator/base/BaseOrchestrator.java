...
public abstract class BaseOrchestrator<T> implements EventHandler, Orchestrator {
  ...
  protected void markSagaComplete(final Event event, final Saga saga, final T sagaData, final String payloadToSubscribers) {
    ...
    if (this.shouldSendNotificationEvent) {
      ...
      if(saga.getSdcSchoolStudentID() != null) {
        finalEvent.setSdcSchoolStudentID(saga.getSdcSchoolCollectionID().toString());
      }
  ...
  @Override
  @Transactional
  public Saga createSaga(@NotNull final String payload, final UUID sdcSchoolStudentID, final String userName) {
