...
public class AdultOLGradeRule implements ValidationBaseRule {
    ...
    private boolean onlineConditionPassed(SdcStudentSagaData sdcStudentSagaData) {
        return  (sdcStudentSagaData.getSchool().getFacilityTypeCode().equalsIgnoreCase(FacilityTypeCodes.DISTONLINE.getCode()) || sdcStudentSagaData.getSchool().getFacilityTypeCode().equalsIgnoreCase(FacilityTypeCodes.DIST_LEARN.getCode())) &&
                !SchoolGradeCodes.getAllowedAdultGrades().contains(sdcStudentSagaData.getSdcSchoolCollectionStudent().getEnrolledGradeCode());
    }
}
