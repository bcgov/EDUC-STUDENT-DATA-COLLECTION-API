...
public interface SdcSchoolCollectionStudentRepository extends JpaRepository<SdcSchoolCollectionStudentEntity, UUID>, JpaSpecificationExecutor<SdcSchoolCollectionStudentEntity> {
  ...
  @Query(value = """
        ...
        COUNT(DISTINCT CASE WHEN sscs.specialEducationCategoryCode in ('A','B') and s.isSchoolAged = true AND scss.fte > 0 THEN sscs.sdcSchoolCollectionStudentID END) as specialEducationLevel1Count,
        COUNT(DISTINCT CASE WHEN sscs.specialEducationCategoryCode in ('C','D','E','F','G') and s.isSchoolAged = true AND scss.fte > 0 THEN sscs.sdcSchoolCollectionStudentID END) as specialEducationLevel2Count,
        COUNT(DISTINCT CASE WHEN sscs.specialEducationCategoryCode in ('H','J','M','N') and s.isSchoolAged = true AND scss.fte > 0 THEN sscs.sdcSchoolCollectionStudentID END) as specialEducationLevel3Count,
        COUNT(DISTINCT CASE WHEN sscs.specialEducationCategoryCode in ('K','P','Q','R') and s.isSchoolAged = true AND scss.fte > 0 THEN sscs.sdcSchoolCollectionStudentID END) as specialEducationLevelOtherCount,
