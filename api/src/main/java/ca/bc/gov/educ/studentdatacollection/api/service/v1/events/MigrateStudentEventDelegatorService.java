...
@Service
@Slf4j
public class MigrateStudentEventDelegatorService implements EventHandler {
  /**
   ...
   */
  @Autowired
  public MigrateStudentEventDelegatorService(final EventHandlerService eventHandlerService, final EventPublisherService eventPublisherService) {
  ...
  public void handleEvent(final Event event) {
    try {
      if(event.getEventType() == EventType.PROCESS_SDC_MIGRATION_STUDENT) {
        log.debug("Received read from topic event :: ");
        log.trace(PAYLOAD_LOG, event.getEventPayload());
        this.getEventHandlerService().handleMigrateStudentEvent(event); // no response in this event.
      }
      else {
  ...
  public String getTopicToSubscribe() {
    return TopicsEnum.MIGRATE_STUDENT_TOPIC.toString();
  }
