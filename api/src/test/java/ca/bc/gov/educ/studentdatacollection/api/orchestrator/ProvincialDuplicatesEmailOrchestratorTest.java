...
@SpringBootTest
@ActiveProfiles("test")
class ProvincialDuplicatesEmailOrchestratorTest extends BaseStudentDataCollectionAPITest {

  @MockBean
  protected RestUtils restUtils;
  @Autowired
  ProvincialDupliatesEmailOrchestrator provincialDupliatesEmailOrchestrator;
  @Autowired
  ...
  void testHandleEvent_givenEventTypeInitiated_shouldExecuteEmailSendWithEventOutComeCOMPLETED() {
    ...
    emailFields.put("schoolName", "Test School");
    final EmailSagaData sagaData = EmailSagaData
            .builder()
            .fromEmail("abc@acb.com")
            .toEmails(Arrays.asList("someone@somewhere.com"))
            .subject("Test")
            .templateName("collection.provincial.duplicates.notification")
            .emailFields(emailFields)
            .build();

    val saga = this.createMockProvincialDuplicatesEmailSaga(sagaData);
    saga.setSagaId(null);
    ...
    this.provincialDupliatesEmailOrchestrator.handleEvent(event);
    ...
    verify(this.messagePublisher, atMost(2)).dispatchMessage(eq(this.provincialDupliatesEmailOrchestrator.getTopicToSubscribe()), this.eventCaptor.capture());
