...
class MinistryReportsControllerTest extends BaseStudentDataCollectionAPITest {
  ...
  @Test
  void testGetMinistryReportCSV_TypeALL_STUDENTS_EXPORT_ShouldReturnReportData() throws Exception {
    final GrantedAuthority grantedAuthority = () -> "SCOPE_READ_SDC_MINISTRY_REPORTS";
    final OidcLoginRequestPostProcessor mockAuthority = oidcLogin().authorities(grantedAuthority);
    ...
    var resultActions1 = this.mockMvc.perform(
                    get(URL.BASE_MINISTRY_HEADCOUNTS + "/" + collection.getCollectionID() + "/all-students-export/download").with(mockAuthority))
            .andDo(print()).andExpect(status().isOk());
    ...
    assertThat(summary1).isNotNull();
    assertThat(summary1.getReportType()).isEqualTo(ALL_STUDENTS_EXPORT.getCode());
  }
