...
@SpringBootTest
@ActiveProfiles("test")
class IndySchoolsNoActivityOrchestratorTest extends BaseStudentDataCollectionAPITest {

  @MockBean
  protected RestUtils restUtils;
  @Autowired
  IndySchoolNoActivityEmailOrchestrator indySchoolNoActivityEmailOrchestrator;
  @Autowired
  ...
  void testHandleEvent_givenEventTypeInitiated_shouldExecuteValidateStudentWithEventOutComeVALIDATION_SUCCESS_WITH_ERROR() {
    ...
    val saga = this.createMockIndyNoActivityEmailSaga(sagaData);
    saga.setSagaId(null);
    ...
    this.indySchoolNoActivityEmailOrchestrator.handleEvent(event);
    ...
    verify(this.messagePublisher, atMost(2)).dispatchMessage(eq(this.indySchoolNoActivityEmailOrchestrator.getTopicToSubscribe()), this.eventCaptor.capture());
